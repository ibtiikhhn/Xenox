...
import com.globalsolutions.Tattle.Utils.ChatPingAlarmManager;
...
public class LoginService extends Service {
    ...
    private void startLoginToChat() {
        if (chatService.isLoggedIn()) {
            Log.i(TAG, "chatservice is logged in: ");
            sendResultToActivity(true, null);
        } else {
            Log.i(TAG, "startLoginToChat: loggin in to chat");
    ...
    private void loginToChat(QBUser qbUser) {
        chatService.login(qbUser, new QBEntityCallback<QBUser>() {
            ...
            public void onSuccess(QBUser qbUser, Bundle bundle) {
                Log.i(TAG, "ChatService Login Successful");
                startActionsOnSuccessLogin();
            }

            @Override
            public void onError(QBResponseException e) {
                Log.i(TAG, "ChatService Login Error: " + e.getMessage());
